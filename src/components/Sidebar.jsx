...
const Sidebar = ({ role, onClose }) => {
  ...
  const navigation = userRole === 'owner' ? ownerNavigation : employeeNavigation;

  // Filter navigation items based on user role
  const filteredNavigation = navigation.filter(item => item.role === userRole);

  return (
    <div className={`w-64 h-screen flex flex-col ${colors.card} border-r ${colors.border}`}>
      ...
      <div className="p-4 border-b border-gray-700">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-lg font-bold text-blue-500 tracking-tight">RetailFlow</h1>
              ...
              <Link
                ...
                className={`flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                  isActive
                    ...
                    : `${colors.textSecondary} hover:${isDark ? 'bg-gray-700' : 'bg-gray-100'} hover:${colors.text}`
                }`}
              >
